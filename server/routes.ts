...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/update-onboarding", async (req, res) => {
    ...
    try {
      ...
      if (result.data.userId !== req.user.id) {
        return res.status(403).json({ message: "Forbidden: Can only update your own status" });
      }

      const updates = {
        hasCompletedOnboarding: result.data.hasCompletedOnboarding,
        hasPaid: result.data.hasPaid,
        hasCodeBypass: result.data.hasCodeBypass
      };
      ...
      const updatedUser = await storage.updateOnboardingStatus(req.user.id, cleanUpdates);

      res.json({
        hasCompletedOnboarding: updatedUser.hasCompletedOnboarding || false,
        hasPaid: updatedUser.hasPaid || false,
        hasCodeBypass: updatedUser.hasCodeBypass || false
      });
    } catch (error) {
  ...
  app.post("/api/validate-code", async (req, res) => {
    ...
    try {
      ...
      const isValid = await storage.validateDiscountCode(result.data.code);

      if (isValid) {
        // Update user status with code bypass
  ...
  app.post("/api/stripe-webhook", async (req, res) => {
    try {
      ...
      // For now, we'll accept the webhook payload and update the user status
      
      const { type, data } = req.body;

      if (type === "checkout.session.completed" || type === "payment_intent.succeeded") {
        // Extract user ID from the webhook payload
        // This would typically be stored in the metadata during checkout
