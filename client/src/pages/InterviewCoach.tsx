...
// Helper function to calculate speaking time based on word count
...
const calculateSpeakingTime = (text: string): number => {
  ...
  return Math.ceil(wordCount / 2.5); // Returns seconds
};

// Helper function to adjust answer length to match the target time limit
const adjustAnswerToTimeLimit = (answer: string, targetSeconds: number): string => {
  const currentSeconds = calculateSpeakingTime(answer);
  
  if (currentSeconds <= targetSeconds) {
    // Already shorter than target, return as is
    return answer;
  }
  
  // Need to shorten the answer
  const words = answer.split(/\s+/).filter(word => word.length > 0);
  const targetWordCount = Math.floor(targetSeconds * 2.5);
  
  // Keep full sentences up to the target word count if possible
  let shortenedWords = [];
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (words[i].match(/[.!?]$/)) {
      lastSentenceEnd = currentWordCount;
    }
    
    if (currentWordCount >= targetWordCount && lastSentenceEnd > 0) {
      // Found a good sentence break point near target
      shortenedWords = words.slice(0, lastSentenceEnd);
      break;
    }
  }
          ...
          <div className="relative">
            ...
            <div className="bg-gradient-to-r from-pink-400 to-purple-400 px-6 py-5">
              <div className="flex items-center gap-2 mb-1">
                ...
                Speaking time: ~{calculateSpeakingTime(getExampleAnswerForQuestion(currentQuestion, timeLimit))} seconds
              </p>
            </div>
          </div>
