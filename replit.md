...
- July 23, 2025. Created comprehensive Privacy Policy page at /privacy route. Added formal, easy-to-read privacy policy with proper sections covering data collection, usage, security, and user rights. Integrated contact information for Ojas Kandhare (okandy@uw.edu) and Arshia Kathpalia (arshia.x.kathpalia@gmail.com). Removed age restriction (13+) as app is accessible to all users. Added link to external privacy policy generator. Page is publicly accessible without authentication, hides global dock, and features professional design consistent with Runway AI branding.
- July 23, 2025. Updated authentication page with new Runway AI logo. Replaced airplane emoji with professional crown-style Runway AI logo featuring pink crown and "AI" text in circle. Logo displays on white background for proper contrast and branding consistency across the application.
- July 24, 2025. Completely redesigned Dress Try-On page with cool tilted card interface. Implemented progressive reveal where "Upload Picture of Yourself" card shows first (tilted left), then "Choose Your Dress" card appears after first upload (tilted right, overlapping). Cards are positioned side by side with dramatic shadows and smooth animations. Added "See the Magic" button that triggers a 3-second sparkle animation before processing. Enhanced overall visual appeal with larger cards, bold typography, and improved user flow guidance.
- July 24, 2025. Enhanced dashboard with Cal AI-inspired redesign. Implemented exact 7-day engagement tracker with clean white card design, green circles for accessed days, and date numbers. Added functional + button to "Coming Up" section for adding calendar events. Created functional photo cluster with overlapping design (-ml-3 spacing), file upload handling, and mock photo URLs for development. Fixed navigation and authentication issues for seamless user experience.
- July 30, 2025. Completely redesigned image board modal for enhanced user experience. Made modal significantly larger and more immersive (max-w-6xl, 95vw) with dramatic rounded corners and shadows. Removed save count display from stats, now showing only likes as heart icon + number for cleaner interface. Added comprehensive action buttons: Share (native sharing API with clipboard fallback), "Search with Google" (opens image search in new tab), enhanced like/save buttons with color-coded borders and hover effects. Fixed all text readability issues in upload dialog with proper dark text colors for maximum contrast on white backgrounds.
